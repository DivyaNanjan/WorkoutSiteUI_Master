...
export class WorkouttxnComponent implements OnInit {
  ...
  createWorkoutTxn() {
    this.loading = true;
    console.log("workout data =>  "+JSON.stringify(this.dataService));
    ...
    var endDate = new Date(this.workoutTxn.endTime).toISOString();
    this.workoutTxn.endTime=endDate.replace('Z','');
